...
function makeInvoiceMatcher(feeTotal?: (ids: string[]) => number) {
  return (inv: Invoice, filter: TableFilter): boolean => {
    ...
    const meta = invoiceFieldMetas.find((m) => m.key === filter.field);
...
const BASE_STORAGE_KEY = "invoice-table-views";
const BASE_ACTIVE_VIEW_KEY = "invoice-table-active-view";
...
export function useInvoiceTableViews(userId?: string) {
  ...
  const applyFiltersAndSorts = useCallback(
    (invoices: Invoice[], feeTotal?: (ids: string[]) => number): Invoice[] => {
      ...
      if (topIds.size > 0 || bottomIds.size > 0) {
        const pt: Invoice[] = [], pb: Invoice[] = [], mid: Invoice[] = [];
