...
- **[`.cursor/rules/architecture.mdc`](.cursor/rules/architecture.mdc) §11** — **全域常駐**：客戶／正式案件檔（`.sdlxliff`／`.mqxliff`／`.xliff` 等）絕不得放入 repo 或提交；僅去識別化最小樣本可放 `tests/fixtures/`。
...
- [`docs/CAT_TB_EDITOR_LIVE_SYNC_PLAN_2026-06.md`](docs/CAT_TB_EDITOR_LIVE_SYNC_PLAN_2026-06.md) — 術語庫改刪**即時同步**、footer 底列**編輯／刪除／隱藏**（團隊版 UUID 熱修 `22ba4b1`；**已驗收**）
...
- [`docs/CAT_LARGE_FILE_VIRTUAL_SCROLL_NAV_DEVLOG_2026-07.md`](docs/CAT_LARGE_FILE_VIRTUAL_SCROLL_NAV_DEVLOG_2026-07.md) — **大檔虛擬捲動與導覽完整開發紀錄**（2026-05～07 敘事型彙整；Phase R 已達驗收門檻 `21af736`；整合 7 份來源文件，附時間軸與 commit 表）
- [`docs/CAT_EDITOR_TAG_COLOR_AND_NAV_FIX_2026-06.md`](docs/CAT_EDITOR_TAG_COLOR_AND_NAV_FIX_2026-06.md) — **Phase 2.3** … + **2.3o 審稿外圈 2.5px 待驗收**（§3.17）；**2.3q 已實作 `6344baa`**（§3.18）；**Playwright Phase R 已達驗收門檻** `21af736`（[`CAT_EDITOR_NAV_PHASE_2_3Q_PLAYWRIGHT_PLAN.md`](docs/CAT_EDITOR_NAV_PHASE_2_3Q_PLAYWRIGHT_PLAN.md)）；完整脈絡見上條 DEVLOG；2.3p hotfix `649ef70`；2.3n 已通過（§3.15–§3.16）；2.3k／2.3l 部分項仍見 §3.11／§3.13 — 色點見 [`CAT_SEGMENT_USER_MARKERS_2026-06.md`](docs/CAT_SEGMENT_USER_MARKERS_2026-06.md)
...
- [`docs/CAT_LMS_ASSIGN_REDESIGN_PLAN_2026-06.md`](docs/CAT_LMS_ASSIGN_REDESIGN_PLAN_2026-06.md) — **派案介面與同步重構**（**規劃中**）：CAT 指派視窗升級為協作式表格（角色／列範圍／交期）、拆分接指派、淘汰 `cat_file_assignments` 統一 `cat_stage_assignments`、過度同步、譯者 UUID 雙寫配對、同步失敗不靜默；靜態預覽 [`docs/preview-cat-assign/index.html`](docs/preview-cat-assign/index.html)
