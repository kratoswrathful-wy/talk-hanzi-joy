...
**實作備註（B2）**：比對列 `ondblclick` 改為 `handleCatResultDblClick(this)`；依既有 `data-index` 與 `catMatchPageIndex` 換算絕對列後，自 `currentTmMatches` 讀取 `targetText` 再呼叫 `handleCatResultApply`。鍵盤套用仍走 `applyCatMatchAtIndex`（記憶體字串），無 inline 內嵌譯文。
...
- **B 階段**：雙擊已改為不依賴 inline 內嵌譯文；鍵盤套用仍須確認與 `applyCatMatchAtIndex` 一致（見 §8）。
...
以下可**分開**驗收：A 未上線時，仍可單獨勾 B；B 已上線時，A 上線後建議再跑一輪 B 的煙霧測試（同檔 `app.js`）。

### 階段 A — Bug #5（mqxliff 部分譯文 targetTags、F8）

1. **匯入**：使用「譯文 `<target>` 僅含部分 `<ph>`、其餘 ph 只在原文」的 mqxliff，開檔後該句段 `targetTags` 筆數（或 `ph` 編號覆蓋）與原文 **一致或可對齊**，無無故缺號。
2. **F8**：在譯文欄用 F8 補齊缺漏 tag 後，觸發搜尋／篩選重畫、譯文欄失焦寫庫、**關檔再開**同一專案，補上的句段仍顯示 **pill**，不得退化成純文字 `{3}` 等。
3. **Team 模式**（若有使用）：確認寫庫後遠端／`target_tags` 與 xliff-tag-export 規則一致，無漏欄。

### 階段 B — TM 比對表 HTML（Bug #6）

1. **顯示**：準備或挑一筆 TM **譯文**含雙引號的內容（例如字面 `<img id="Tool2"/>` 或 `id="Jump"`），開啟 CAT 分頁比對表 → **四欄排版正常**，索引欄不得出現 `0)">`、屬性殘段或整欄直排擠成一條的破版。
2. **雙擊套用**：雙擊該列後，作用句段（目前 active 列）譯文應替換為該 TM 譯文全文，**含**原先會破壞 HTML 的 `"` 等字元；比對分數列視覺（若有）仍合理。
3. **鍵盤套用**：在 CAT 分頁選取含雙引號譯文之列，用既有快速鍵套用（與 `applyCatMatchAtIndex` 相同路徑）→ 行為與雙擊一致，無錯誤 toast。
4. **分頁**：比對結果超過 9 筆時換頁（Alt+←／→）後，於**非第一頁**雙擊套用 → 應套用**該頁對應列**之 TM，而非錯列。
