...
> 與分階段重建總覽的關係：見 [CAT-phased-rebuild-audit.md](./CAT-phased-rebuild-audit.md)（較偏進度稽核）；**本文件**偏 **行為、資料表、部署與 AI 組字**。**議題群組**之產品與技術權威見 **§5.6**。**本主題之後續波次（範例送 prompt、alert 收斂等）** 之**建議順序**見 **§13**。
...
5. **429/速率過快風險（待落地）**：
   - 現況已有 retry（exponential backoff + jitter），但仍可能在大批次觸發速率上限。
   - 下一步以「不破壞現有流程」優先：先支援 `Retry-After`、再加批次 cooldown、再補全域 AI 任務互斥與連續 429 降載。
   - 若啟用上述策略，需在驗收中覆蓋「長檔連續多批 + 人為 429 模擬 + 可恢復重跑」。
...
本節所述 **MVP 已落地**（`cat-tool`、團隊 RPC、離線合併）。後續接軌**議題群組**、**拖曳排序**、**可選：送進 prompt** 之優先序見 **§13**。
...
- **MVP 預設不將範例送入** `buildPrompt`／掃描 prompt：範例作維運與審稿輔助。產品若要啟用，見 **§13 階段 C**。
...
### 12.2 速率控制策略（定稿，待程式落地）

> 目標：降低 429（`rate_limit_exceeded`）導致的「翻譯失敗：請求速率超過上限，請稍後再試」。

- **現況（已在程式）**：`cat-tool/js/ai-translate.js` 已有 retry（exponential backoff + jitter）且 `insufficient_quota` 不重試。
- **本輪定稿新增（待實作）**：
  1. 429 優先尊重 `Retry-After`（有 header 先用 header）。
  2. 批次間固定 cooldown（避免成功批次連發觸發 RPM）。
  3. 全域 AI 任務互斥（避免掃描與翻譯同時搶配額）。
  4. 連續 429 自動降載（減少每批行數/字元，保守重試）。
  5. 最終失敗時保留可恢復重跑上下文（避免整輪歸零）。
...
- [ ] 1,000+ 字與 30,000 字情境皆能自動拆批並逐批送出。
- [ ] 遇 429 時不立即整輪失敗，能看到重試與節流效果。
- [ ] 「參照來源（附估計 token 數）」顯示純數字；其他區維持「約 xx tokens」。
- [ ] 修改案例可開啟 AI 學習範例庫、勾選（<=15）並回填。
- [ ] 全文/指定範圍切換與驗證阻擋行為符合定稿。
...
| **C** | **AI 批次翻譯介面重製**（規格定稿見 **§12**）：範圍切換、參照來源、修改案例庫、token 呈現口徑與驗證文案 | **已完成預覽定稿，待程式落地** |
| **（加值）** | 範例卡 **拖曳排序**（陣列順序、無額外 schema 變更） | 次優；可併在 **C** 之後或與 **B** 錯峰 |
| **D** | **速率保護落地**：`Retry-After`、批次 cooldown、AI 任務互斥、連續 429 降載、可恢復重跑 | 先做最小可行（MVP）再逐步增強 |
