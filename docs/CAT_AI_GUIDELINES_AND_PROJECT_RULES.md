...
| `cat_ai_project_settings` | 每專案：已選準則 ID、文風 ID、`special_instructions`（檔案特殊指示）、`project_ai_instructions`（專案 AI 指示）、`project_guidelines`（見第 5 節）、更新時間等；見 [CAT_AI_FILE_SPECIAL_VS_PROJECT_INSTRUCTIONS_PLAN.md](./CAT_AI_FILE_SPECIAL_VS_PROJECT_INSTRUCTIONS_PLAN.md) |
...
5. **狀態語意與刷新邊界（本次補充收錄）**  
   - 現況（`localStorage` 初版）下，若頁面刷新、關閉分頁或前端執行脈絡中斷，進行中的任務實際上會停止，但紀錄可能暫留「進行中」。  
   - 本期先將其視為**已知限制**：`running` 僅代表「該次前端流程曾進入執行狀態」，**不保證**跨刷新後仍持續。  
   - `DECISION_PENDING:` 是否在下次載入時，將未正常 `finish` 的 `running` 自動改標為 `interrupted`／`aborted`，並補上中斷時間與原因來源（refresh/close/crash）。

6. **平台異常排除邊界（本次補充收錄）**  
   - 驗收與問題判讀時，若觀察到 Supabase 專案 `unhealthy`、HTTP 522、暫時性 CORS/連線異常，優先歸類為**平台狀態事件**，不直接視為本波前端 UX（toast/任務紀錄）變更造成。  
   - `VALIDATION_PENDING:` 平台恢復後需以同一操作路徑重跑回歸，確認「任務啟動、進度更新、完成/失敗落記」是否可穩定重現，再決定是否開前端修復單。  
   - `RISK:` 若在平台不穩期間直接驗收，易把基礎設施波動誤判為產品邏輯缺陷，導致錯誤優先序。
...
| 2026-04-30（AI 長任務議題補充收錄） | **§7.1 補強**：補列 `running` 殘留語意與「刷新/關頁即中斷」邊界，明確標註為初版 `localStorage` 已知限制。**平台異常脈絡**：新增 Supabase `unhealthy`／522／暫時性 CORS 之判讀邊界，要求先做平台事件排除再判定前端缺陷。**未定案入口**：以 `DECISION_PENDING`／`VALIDATION_PENDING`／`RISK` 標記後續決策與驗收關卡，並導向第二波盤點計畫。 |
