...
- **收件人條件（派案端）**：角色為 **PM 或 Executive**，且已在 **個人檔案連結 Slack**（`user_slack_meta`），並開啟 **接收「承接／無法承接」自動 Slack 私訊**；其 **profiles.email** 須能在 Slack 工作區以 `users.lookupByEmail` 找到對應成員。
- **無法承接** 時：個人檔可設定第 2、3 行預設文案；表單填寫期限、字數、補充說明會接在後段一併送出。
