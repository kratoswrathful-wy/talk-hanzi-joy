...
- **收件人條件（派案端）**：角色為 **PM 或 Executive**，且已在 **個人檔案連結 Slack**（`user_slack_meta` 必須有 OAuth 寫入的 **`slack_user_id`**），並開啟 **接收「承接／無法承接」自動 Slack 私訊**。**私訊目的地只使用該成員已登錄的 Slack user id**（即連結 Slack 時授權帳號在該工作區的成員 id），**不依** `profiles.email` 或 `users.lookupByEmail` 路由；故系統註冊信箱與 Slack 帳號信箱不同亦無妨。若未連結 Slack 或 DB 無 `slack_user_id`，該收件人不會列入通知。
