...
## 8. 一進個人檔案連 Slack 就錯、`user_slack_meta` 為 404（PGRST205）、按「連結 Slack」變 500
...
套用後在 **Table Editor** 應可看到 **`user_slack_meta`**、**`slack_oauth_states`** 等表；再重新整理網頁與在**個人檔案**測試「連結 Slack」。
