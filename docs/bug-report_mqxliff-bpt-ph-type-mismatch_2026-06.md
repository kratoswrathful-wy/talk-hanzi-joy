...
2. **pill 顏色**（需 Bug #11 已修之 TM 全 ph 列）：open `{1}`、`{/1}` 與原文 **同編號**且 **藍色**；僅缺 close 時 **橙色**（可 F8 補）。Bug #11 未修前 tu id=1 整列橘／紅屬預期。
...
## 7. Bug #10 邊界（非本 bug 涵蓋）

Bug #10 僅在譯文 **`{N}` 與原文 open `{N}` 同號** 且 `innerEscapedTagSig` 相同時，將 standalone `ph` 覆寫為 bpt，並可補 `{/N}` close。

**未涵蓋**：TM 模糊匹配把整句 bpt/ept **拆成連續 standalone ph** → 譯文佔位變 `{1}{2}{3}…`，close 的 `</titleLeft>` 落在 `{2}` 而非 `{/1}`，與原文 `{1}{/1}{2}{/2}…` **錯位**。樣本 `trans-unit id="1"`／`id="4"` 即屬此類；`id="2"`／`id="3"`（譯文已是 bpt/ept）不受影響。

**延伸修正**：Bug #11 — [`bug-report_mqxliff-tm-ph-sequential-mismatch_2026-06.md`](bug-report_mqxliff-tm-ph-sequential-mismatch_2026-06.md)（`fixMqxliffTmPhSequentialPairs`）。

**驗收修正**：Bug #10 原文「tu id=1 open 藍」在 Bug #11 未修前，close 與後續 tag 仍會橘／紅；屬預期，非 #10 失效。
