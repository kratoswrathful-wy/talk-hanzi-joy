...
**維護邊界（2026-06）**：假游標／捲動提示 DOM 掛載於 `#editorGrid` 內 `#catEditorChromeLayer`（層內 `position: absolute` 座標），不再 append 至 `document.body`。捲動監聽、modal 互斥與匯出 overlay 行為以 [`CAT_EDITOR_OVERLAY_FAKE_CARET_EXPORT_2026-06.md`](./CAT_EDITOR_OVERLAY_FAKE_CARET_EXPORT_2026-06.md) §2.4–§2.6 為準；本檔 §A+B 點擊／`navigateToSegmentBySegId` 邏輯不變。`navigateToSegmentBySegId`／`restore()` 內 `scrollIntoView` 已改 **`behavior: 'auto'`**（`5b5aa3d`，已驗收）；見 [`CAT_SCROLL_INSTANT_NAVIGATION_2026-06.md`](./CAT_SCROLL_INSTANT_NAVIGATION_2026-06.md)。
