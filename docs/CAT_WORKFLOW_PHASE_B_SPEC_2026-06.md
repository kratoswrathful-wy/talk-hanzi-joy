...
> **狀態**：**Phase B 已落地並驗收**（2026-06-15）。B-0～B-5／B-4 v5（`e4a6205`～`d7232ab`）。**B-6 延伸**（檔案準備閘門、審稿任務完成）**已實作**（`fd67332`、migration 已 push）— 見 [`CAT_WORKFLOW_PREP_AND_REVIEW_B6_SPEC_2026-06.md`](./CAT_WORKFLOW_PREP_AND_REVIEW_B6_SPEC_2026-06.md)。**B-7a／B-7b／B-7c 已實作**；**B-7d～e 待實作／待排程** — 見 [`CAT_WORKFLOW_B7_UNIFIED_STATUS_AND_LIST_UX_2026-06.md`](./CAT_WORKFLOW_B7_UNIFIED_STATUS_AND_LIST_UX_2026-06.md)。**後續修補**：開檔熱修 `cee4b03`／`60e8526`（§11.7）、更新作業檔×句段集 Modal `76be7ee`（§11.8）、任務完成按下驗證 `d7232ab`（§11.4.1／§11.9）。  
...
| 議題 | 定案 |
|------|------|
| 步驟定義 | **固定兩步** 翻譯 → 審稿（**B-6 已實作**第 0 步 `prep`，見 §12）；**B-7** 統一顯示語意（見 §13）；**暫不**提供自訂增減步驟 UI |
...
**與 B-7 清單語意落差（已知，待 B-7e）**：§3.3 組合表仍依 Phase B `_isWfTransMarkedEffective` fallback（匯入 `status: 'confirmed'` 在翻譯步驟可視為 T_wf）。B-7 清單「待開始」看 `first_edited_at`，**兩套規則尚未對齊**。調查見 [`bug-report_workflow-import-confirmed-status-column_2026-06.md`](./bug-report_workflow-import-confirmed-status-column_2026-06.md)。
...
| 項目 | 定案 |
|------|------|
| 顯示 | **任務完成** 主按鈕（無 ▾）；有**翻譯**段落指派時顯示（**B-6** 將依開檔 session 分流，審稿 session 顯示審稿待完成指派，見 §12） |
| 可點 | **有待完成翻譯指派時常駐可點**；僅該譯者所有翻譯段落皆 `completed` 時反灰（§11.4.1／`d7232ab`） |
| 按下 | 先驗證受派列號範圍內句段**有效已確認**；通過後 `workflow_status` → `completed`、LMS `taskCompleted=true` |
| 未通過 | toast 說明（含未確認列號範例）；**不**寫入狀態 |
| 編輯 | 完成後**仍可編輯**句段 |
...
- `refreshWfTaskCompleteToolbar` — 顯示待完成**翻譯**指派；**按下時**驗證（§11.4.1），非預先反灰（**B-6** 延伸：審稿 session 改顯示審稿指派，見 §12）
...
| 子項 | 交付物 | 主要觸點 |
|------|--------|----------|
| **B-0** | 排序 spec 落地：檔序、句段集 sort、左欄顯示序、篩選 A；更新檔×句段集 UI | **已落地**（[`CAT_SORT_AND_DISPLAY_ORDER_SPEC_2026-06.md`](./CAT_SORT_AND_DISPLAY_ORDER_SPEC_2026-06.md)；2026-06-15 Modal） |
...
| **B-4** | 派出 RPC、任務完成／調整狀態、雙向同步、開檔 session（§4-bis） | **已落地**（v4～v5；`e4a6205`～`d7232ab`） |
...
| B-4e | 一般人任務完成（v4 反灰；**v5-j** 改按下驗證）；PM「調整狀態」+ Modal | 已落地 |
| B-4f | 雙向退回 + retroactive 補寫 | 已落地 |
| B-4g | v4：replace 同步、協作表 UI、N- 列範圍、LMS 狀態退回、工具列常駐、審稿圖示、collab 遷移 | 已落地 |
| B-4h | v5：`collab_row_id` text、檔案清單 assignment 逐行、LMS 升級、譯者勾選驗證、PM 調整狀態永遠可用 | 已落地 `e4a6205` |
| B-4i | 開檔熱修 TDZ／`allSegments`；工具列 race `92e2208` | 已落地 `cee4b03`、`60e8526` |
| B-4j | 任務完成按下驗證（拆分指派可點、toast 回饋） | 已落地 `d7232ab` |
...
11. **一般人**：有待完成指派時可點「任務完成」；按下驗證全範圍已確認；完成後反灰；LMS 取消→可再點。
...
| 開檔 Console 無 workflow TDZ 錯誤 | 熱修後通過（§11.7） |
| 拆分指派譯者「任務完成」 | 常駐可點；全確認後按下成功；未全確認 toast（`d7232ab`） |
| 更新作業檔×句段集 | 有涵蓋句段集時跳出確認 Modal（§11.8） |
...
#### 11.4.1 一般人（非 PM+）— 按下時驗證（2026-06-15 調整）

**UI**：有翻譯段落指派時「任務完成」**常駐可點**（僅在該指派已全部 `completed` 時反灰）。

**按下時驗證**（與 LMS 勾選任務完成同一心智模型）：

1. 若 `currentWfSessionKind === 'review'` → toast 提示改選翻譯工作，中止。
2. 受派**列號範圍**內所有句段須**有效已確認**（`_isWfTransMarkedEffective`；與狀態欄／進度一致，含舊檔 memoQ 已確認 fallback）。
3. 列號座標與編輯鎖定一致（`_segFullListLineNo`；缺列序時 fallback `globalId`）。
4. 未通過 → toast「尚有未確認句段（例如列 …）」，**不**更新 `workflow_status`、不同步 LMS。
5. 通過 → 維持既有完成流程（單段直接完成；多段開下拉選段落）。

**多段指派**：主按鈕對單段直接驗證＋完成；多段開下拉列出**所有待完成**段落，各段按下時再驗證。

#### 11.4.2 PM 以上

**定案**：「調整狀態」按鈕**永遠可用**（`disabled = false`），無視確認進度；可強制將任何段落設為完成或退回（`e4a6205`／B-4v5-f）。
...
| B-4v5-e | 任務完成：按下驗證受派範圍有效已確認（`_isWfTransMarkedEffective`＋列序）；多段下拉 | `cat-tool/app.js` `_validateAssignmentForTaskComplete`、`d7232ab` |
...
**Git**：`cee4b03`（`allSegments`／`_wfTaskCompleteUiBound`）、`60e8526`（`currentWfSessionKind`）、`92e2208`（工具列 race）。

**驗收**：CCT6012 開檔 749 句；Console 無上述三錯；主管「調整狀態」可用；左上角返回正常。

### 11.8 更新作業檔×句段集確認 Modal（B-0 §5，`76be7ee`）

**背景**：B-0 規格要求更新作業檔後 PM **明確選擇**是否把新句納入句段集；先前 `_syncViewsAfterFileUpdate` 自動依 `filterSummary` 加入，無互動 UI。

**行為**（`fileUpdateViewsModal`）：

| 時機 | 行為 |
|------|------|
| 更新寫入後，檔案涵蓋句段集且有新增／刪除句 | 跳出 Modal |
| 每個句段集 | 顯示名稱、`filter_summary` 文字、將移除之已刪句數 |
| 新句 | 預設勾選符合篩選者；可展開逐句調整；可總開關 |
| 略過加入新句 | 仍自句段集**移除**已刪句段 |
| 多人拆段翻譯（PM+） | 可為各譯者填**新列起迄**，寫入 `upsertTranslateStageAssignment` |

**觸點**：[`cat-tool/index.html`](../cat-tool/index.html) `#fileUpdateViewsModal`；[`cat-tool/app.js`](../cat-tool/app.js) `showFileUpdateViewsModal`、`_syncViewsAfterFileUpdate`、`runFileUpdate`、Excel 更新 wizard；[`cat-tool/db.js`](../cat-tool/db.js)／[`cat-cloud-rpc.ts`](../src/lib/cat-cloud-rpc.ts) `db.upsertTranslateStageAssignment`（RPC `cat_upsert_translate_stage_assignment`）。

**詳細規格**：[`CAT_SORT_AND_DISPLAY_ORDER_SPEC_2026-06.md`](./CAT_SORT_AND_DISPLAY_ORDER_SPEC_2026-06.md) §5、§7。

### 11.9 任務完成按下驗證（`d7232ab`）

**背景**：v5-e 初版以「未全確認則反灰」實作，拆分指派時譯者常見畫面已綠勾但按鈕仍不可用（`wfTransConfirmedAt` 與狀態欄 effective 不一致、或 `globalId` 與列序不一致）。

**定案**（與 LMS 勾選驗證同一心智模型）：

| 項目 | 實作 |
|------|------|
| 按鈕 | `_wfPendingTranslateAssignments` 有待完成指派 → **可點**；全 `completed` → 反灰 |
| 按下 | `_validateAssignmentForTaskComplete` → `_assignmentRangeConfirmAudit` |
| 列範圍 | `_segInAssignmentLineRange`（優先 `_segFullListLineNo`，fallback `globalId`） |
| 已確認 | `_isWfTransMarkedEffective`（與狀態欄／進度一致） |
| 審稿 session | toast 提示改選翻譯身分 |
| 多段 | 主按鈕單段直接驗證；多段開下拉，各段 `_completeWfStageAssignmentById` 內再驗證 |

**驗收**：拆分指派檔（如 CCT6012）譯者全確認後可按任務完成；故意留未確認句時 toast 列號範例。
...
> 本節僅摘要與 Phase B 的差異；實作以子文件為準。

### 12.1 三步驟（prep → translate → review）

| 步驟 | 新匯入預設 | 說明 |
|------|------------|------|
| `prep`（檔案準備） | `active` | PM+「標記準備完成」→ `completed`；**派出閘門**必要條件 |
| `translate` | `pending` | 派出後 `active`（現有 B-4 邏輯） |
| `review` | `pending` | 翻譯完成後 `active`（現有 B-4 邏輯） |

**準備中編輯**：僅 PM+ 可改譯文；譯者／審稿人唯讀。  
**舊檔**：已派出或翻譯已啟動 → migration backfill `prep=completed`。

### 12.2 審稿任務完成（補 Phase B 缺口）

| 角色 | 現行 Phase B | B-6 新增 |
|------|--------------|----------|
| 一般人 | 僅翻譯「任務完成」 | 審稿 session + 審稿段落指派 → 任務完成（驗證 `_isWfReviewMarkedEffective`） |
| PM+ | 調整狀態：翻譯執行中／翻譯完成 | 增加**審稿執行中／審稿完成**（整檔與拆段 Modal） |

**不變**：審稿完成**不**發 Slack；**不**改 LMS `task_completed`（仍僅翻譯協作列）；**暫不**單人多檔審稿聚合。

### 12.3 Phase C 鋪路

- **基準快照**觸發點：`prep` 準備完成（`baseline_before_translate`）。
- B-6 落地時預留 `enqueueStageSnapshot` no-op；Phase C 再寫入 `cat_segment_stage_snapshots`。

### 12.4 與波次 A 的關係

**波次 A 已落地**（`fd67332`）：翻譯完成 Slack、LMS 協作表順序、單人多檔聚合；見 B-6 子文件 §9。

---

## 13. B-7 延伸摘要（B-7a 已實作；B-7b～d 待實作）

> **完整規格**：[`CAT_WORKFLOW_B7_UNIFIED_STATUS_AND_LIST_UX_2026-06.md`](./CAT_WORKFLOW_B7_UNIFIED_STATUS_AND_LIST_UX_2026-06.md)  
> **驗收後修訂**（2026-06-17）：§2.2 文案、PM 六階畫面＋寫入、審稿提前改句防呆。

### B-7a 已落地（`b577c08`）

- Migration：`first_edited_at`；`wf-display-status.js`；清單「指派對象」grid；譯文首次改動寫入 RPC。
- **文案仍為舊版**（如「翻譯待開始」）→ 待 **B-7b** 對齊「待開始／進行中／完成」。

### 驗收後定案（待 B-7b～d）

| 議題 | 定案 |
|------|------|
| 右欄文案 | 待開始／進行中／完成；等待準備完成、等待翻譯完成 |
| PM 階梯 | 六階：準備中→準備完成→翻譯執行中→翻譯完成→審稿執行中→審稿完成；**畫面＋寫入**連動 |
| 審稿提前改句 | 允許；僅審稿列進行中，**不**自動標翻譯完成 |
| 準備完成 | `openCatConfirmModal`；以最後一次準備完成為 `baseline_before_translate` |
| **B-7d** | CAT 指派同步 `cat_stage_assignments`；檔內／句段集內列號；見 [bug-report](./bug-report_workflow-whole-file-assign-edit-lock_2026-06.md) |
| **B-7c** | 儀表板、`cat_file_user_access`；停用 `in_progress` |
...
| 2026-06-15 | **B-4 v5 實作**：`collab_row_id` text migration、檔案清單 assignment 逐行顯示、LMS 升級條件擴充、譯者勾選驗證、PM 調整狀態永遠可用 |
| 2026-06-15 | **開檔熱修** `cee4b03`／`60e8526`／`92e2208`：§11.7；CCT6012 開檔驗收 |
| 2026-06-15 | **B-0 更新作業檔×句段集 UI** `76be7ee`：§11.8；`fileUpdateViewsModal` |
| 2026-06-15 | **任務完成按下驗證** `d7232ab`：§11.4.1、§11.9；拆分指派驗收通過 |
| 2026-06-15 | **B-6 延伸摘要** §12：prep 閘門、審稿任務完成、Phase C 基準觸發；子文件 [`CAT_WORKFLOW_PREP_AND_REVIEW_B6_SPEC_2026-06.md`](./CAT_WORKFLOW_PREP_AND_REVIEW_B6_SPEC_2026-06.md) |
| 2026-06-16 | **B-6 已實作** §12：`fd67332`、migration 已 push；§13 **B-7** 延伸摘要（統一狀態、清單／儀表板 UX） |
| 2026-06-17 | **B-7a 已實作** §13：`b577c08`；驗收後修訂文案、PM 六階、B-7d 鎖定；[`bug-report_workflow-whole-file-assign-edit-lock_2026-06.md`](./bug-report_workflow-whole-file-assign-edit-lock_2026-06.md) |
| 2026-06-17 | **B-7b 已實作** `e313016`；§3.3 與 B-7 清單落差、**B-7e** 待排程；[`bug-report_workflow-import-confirmed-status-column_2026-06.md`](./bug-report_workflow-import-confirmed-status-column_2026-06.md) |
