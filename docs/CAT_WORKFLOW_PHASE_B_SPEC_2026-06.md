...
> **狀態**：B-0～B-4／**B-5 已落地**（B-4 v4：replace 同步、協作表 UI、列範圍 `N-`、LMS 狀態退回、工具列常駐、審稿圖示修正、collab_rows 遷移）。**B-4 v5 規劃中**（collab_row_id Bug 修正、檔案清單顯示改版、CAT→LMS 狀態升級、任務完成條件強化）。  
...
- **動作**：向 `CatToolPage` 發送新 postMessage `CAT_WF_FILE_TRANSLATE_COMPLETED`（含 `fileId`、`caseId`）；`CatToolPage` 收到後呼叫新函式將案件狀態升為 `task_completed`。
- **限制**：若 LMS 案件狀態已是 `task_completed`、`delivered` 等，**不改動**。

#### 11.3.2 拆段檔案（全部分段完成）

- **觸發**：任一分段 `workflow_status` 更新後，檢查**所有**分段是否均為 `completed`（CAT 端在 `_pmApplySplitAdjustStatus` 或 `_completeWfStageAssignmentById` 之後）。
- **條件**：所有與該檔 `file_id` 關聯的 `cat_stage_assignments`（翻譯步）均為 `completed`，**且** LMS 案件 `status` 為 `draft`、`inquiry` 或 `dispatched`。
- **動作**：同 11.3.1，發送 `CAT_WF_FILE_TRANSLATE_COMPLETED`。
...
- **驗證機制**：LMS 在勾選時透過 Supabase 查詢 `cat_segments`，篩出 `global_id` 在行號範圍內且 `wf_trans_confirmed_at IS NULL` 的句段數量。若 > 0，彈出 tooltip／toast「尚有未確認句段，無法勾選」並中止。
...
### 11.5 新 LMS 函式：`cat-wf-lms-sync.ts` 擴充

| 函式 | 用途 |
|------|------|
| `upgradeToTaskCompletedIfEligible(supabase, caseId)` | 若案件 status 為 draft/inquiry/dispatched 則升為 task_completed；否則不動 |
| `handleFileTranslateCompleted(supabase, fileId, caseId)` | 接收 `CAT_WF_FILE_TRANSLATE_COMPLETED`，呼叫上述函式 |

`CatToolPage.tsx` 新增處理 `CAT_WF_FILE_TRANSLATE_COMPLETED` postMessage 的分支。
...
| B-4v5-c | CAT→LMS 狀態升級（整檔 + 全分段完成觸發）| `cat-tool/app.js`、`CatToolPage.tsx`、`cat-wf-lms-sync.ts` |
| B-4v5-d | LMS 任務完成勾選：譯者確認驗證（查 `cat_segments`）+ 取消退回 | `src/components/CollaborationTable.tsx` |
...
| 2026-06-15 | **B-4 v5 規劃**：`collab_row_id` 型別 Bug 確認（非 UUID 字串導致 CAT→LMS 同步全斷）、檔案清單 assignment 逐行顯示、CAT→LMS 狀態升級、任務完成條件強化（Dexie 句段確認掃描）；§11 完整規格 |
