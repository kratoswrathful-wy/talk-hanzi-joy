...
> **狀態**：**Phase B 已落地並驗收**（2026-06-15）。B-0～B-5／B-4 v5（`e4a6205`～`d7232ab`）。**B-6 延伸**（檔案準備閘門、審稿任務完成）**已實作**（`fd67332`、migration 已 push）— 見 [`CAT_WORKFLOW_PREP_AND_REVIEW_B6_SPEC_2026-06.md`](./CAT_WORKFLOW_PREP_AND_REVIEW_B6_SPEC_2026-06.md)。**B-7**（統一顯示狀態、檔案清單／儀表板 UX）**規劃中** — 見 [`CAT_WORKFLOW_B7_UNIFIED_STATUS_AND_LIST_UX_2026-06.md`](./CAT_WORKFLOW_B7_UNIFIED_STATUS_AND_LIST_UX_2026-06.md)。**後續修補**：開檔熱修 `cee4b03`／`60e8526`（§11.7）、更新作業檔×句段集 Modal `76be7ee`（§11.8）、任務完成按下驗證 `d7232ab`（§11.4.1／§11.9）。  
...
- **統一顯示**：儀表板與檔案清單共用 `resolveAssignmentDisplayStatus`；**翻譯／審稿待開始** = 至受派範圍內**首次改譯文**前（含派出後僅開檔）。
- **清單 UX**：「指派對象」兩列 grid；僅紅字「準備中」；PM 調整狀態含準備中／翻譯待開始；離開閘門。
- **資料**：`cat_stage_assignments.first_edited_at`、`cat_file_user_access`；`cat_file_assignments.status` 不再驅動 UI。
