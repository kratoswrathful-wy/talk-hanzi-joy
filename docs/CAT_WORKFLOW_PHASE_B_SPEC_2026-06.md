...
> **狀態**：**Phase B 已落地並驗收**（2026-06-15）。B-0～B-5／B-4 v5（`e4a6205`～`d7232ab`）。**B-6 延伸**（檔案準備閘門、審稿任務完成）見 [`CAT_WORKFLOW_PREP_AND_REVIEW_B6_SPEC_2026-06.md`](./CAT_WORKFLOW_PREP_AND_REVIEW_B6_SPEC_2026-06.md)（**規劃中**）。**後續修補**：開檔熱修 `cee4b03`／`60e8526`（§11.7）、更新作業檔×句段集 Modal `76be7ee`（§11.8）、任務完成按下驗證 `d7232ab`（§11.4.1／§11.9）。  
...
| 議題 | 定案 |
|------|------|
| 步驟定義 | **固定兩步** 翻譯 → 審稿（**B-6 規劃**插入第 0 步 `prep`，見 §12）；**暫不**提供自訂增減步驟 UI |
...
建議先實作翻譯完成 Slack／單人多檔修補（與 B-6 無關），再實作 B-6；見子文件 §9。
