...
> **狀態**：**已實作**（待產品端驗收）；**virt 持久化** Phase 2.3 **`0670242`** + **2.3b** `694fa81` + **2.3c～2.3g** — 見 [`CAT_EDITOR_TAG_COLOR_AND_NAV_FIX_2026-06.md`](./CAT_EDITOR_TAG_COLOR_AND_NAV_FIX_2026-06.md) §2.2b／§2.6～§2.10
...
### 2.16 Phase 2.3g — editing preserve 與假游標互斥（2026-06-29）

**問題**（2.3f `927ceec`）：假游標不繪製；離屏 tip 不出現；editing preserve 還原譯文格焦點後 `show()` 因 `activeElement === editor` 而 `hide()`。

**修正**：

- `_suspendEditingPreserve`：TM／右欄 `mousedown` 或譯文格 `blur` 開頭設 true；`restore`／假游標 `show` 成功後清除。
- `captureEditingFocusBeforeVirtRender`：suspend 中不擷取 editing preserve。
- blur：**await 寫庫前**同步 `saveFromSelection`／`setSavedCaret`。
- `refreshAfterVirtRender`：僅當 `saved` 存在且 `activeElement` 非該句譯文格時 `show()`。

**驗收**：§1.3 項 11、19；項 14 regression。
