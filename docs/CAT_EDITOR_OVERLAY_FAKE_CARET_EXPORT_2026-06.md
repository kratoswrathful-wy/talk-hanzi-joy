...
> **狀態**：**Phase 2.3k 規劃中**；**virt 持久化** Phase 2.3 **`0670242`** + **2.3b** `694fa81` + **2.3c～2.3j**（`e147c10`）+ **2.3k** tip 分流 — 見 [`CAT_EDITOR_TAG_COLOR_AND_NAV_FIX_2026-06.md`](./CAT_EDITOR_TAG_COLOR_AND_NAV_FIX_2026-06.md) §2.2b／§2.6～§2.14
...
### 2.20 Phase 2.3k — tip 分流與 listIdx 統一上下（2026-06-30，規劃中）

**問題**（2.3j 後）：真游標在格內捲動時「游標」與「暫存游標」tip 閃爍；游標在視窗上方句段往下捲時 tip 錯誤貼底。

**修法**（[`cat-fake-caret.js`](../cat-tool/js/cat-fake-caret.js)）：

- `resolveOffScreenDirection(segId)`：以 `getSegListIndex` vs `getVirtWindowStartIndex` 判斷上／下；windowStart 無效時保守 `'above'`。
- 真游標在編輯區且 segId 與 saved 相同 → 只 `showRealCaretTipIfNeeded`。
- `show`／`showOffScreenFakeTip` 離屏上下改走 `resolveOffScreenDirection`。
