...
使用者已明確指定「大世界 260908B」為正式**唯讀診斷**案例，用來理解與比對故障。**這不是唯一可檢查的案件，也不是只修這一案。** 修正適用所有符合既定角色、指派與案件狀態條件的案件；程式不得硬編碼特定案件 ID、帳號或姓名。Riot - LoL 260904C 僅作不同原因的對照。Pinnacle 不是本項驗收條件。

隔離環境以合成帳號與合成案件重現、修改及驗收通用行為；正式案件只讀。
...
修法：列表／詳情改呼 `createWithOutcome`，四種結果都有提示；已建立／不明結果保留同一 UUID 且防連點。這修的是吞錯與重複建案，不宣稱已解決列表逾時。使用者當次正式操作究竟是拒絕、逾時還是回應遺失，若無對應請求／回應證據，仍標未知。
