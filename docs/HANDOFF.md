...
## CAT：句段集（cat_views）— 2026-05

句段集讓 PM 將跨多個 XLIFF 檔案的句段彙整為一個工作清單，可單獨指派給譯者。
...
| 表                      | 說明                                                                                                                         |
| ---------------------- | -------------------------------------------------------------------------------------------------------------------------- |
| `cat_views`            | 句段集本體：`project_id`、`owner_user_id`、`name`、`file_ids uuid[]`、`segment_ids uuid[]`、`filter_summary jsonb`、`file_roles jsonb` |
| `cat_view_assignments` | 指派記錄：`view_id`、`assignee_user_id`、`status`、`assigned_by`                                                                   |


Migration：`supabase/migrations/20260503150000_cat_views.sql`。  
RLS：所有已登入使用者可讀 `cat_views`；PM+ 可管理 `cat_view_assignments`；被指派者可讀自己的指派記錄並更新 status。

### RPC（`src/lib/cat-cloud-rpc.ts`）

`listViews` / `getView` / `createView` / `updateView` / `deleteView`  
`getSegmentsByIds`（以 UUID 陣列直接載入句段，分批 200 個）  
`getSegmentsByFileForPreview`（以 file_ids 取句段，內有雙重分頁迴圈：外層 20 個 file 一批、內層每批 `.range()` 最多 1000 筆直到無資料，**徹底解除 Supabase 1000 筆截斷**）  
`listViewAssignments` / `assignView` / `unassignView` / `listMyViewAssignments` / `updateViewAssignmentStatus`

### UI（cat-tool）

- **專案詳情頁** 新增「檔案清單 ↔ 句段集」分頁（`btnProjectTabFiles` / `btnProjectTabViews`）
- **句段集清單**（`panelProjectViews`）
  - 表頭：全選 checkbox、序號、名稱（固定寬 `white-space:nowrap`）、涉及檔案（`max-width:260px`，每行顯示一個檔名、單行截斷 `text-overflow:ellipsis`）、篩選條件、建立者、建立時間
  - 「更名」按鈕位於名稱欄**名稱下方**靠左
  - 工具列批次按鈕（PM-only，無勾選時 `disabled`）：**指派 / 移除 / 分析 / 拆分**
  - Shift+click 範圍勾選（通用 `_setupShiftRangeSelect` helper，同時套用於檔案清單）
- **建立句段集精靈**（`createViewModal`）
  - 進入點：檔案清單工具列「建立句段集」按鈕（有勾選帶入勾選檔案，無勾選帶入全部）
  - 模式：**快速結合**（顯示涉及檔案清單，每行一個檔名 + 捲軸）vs **自訂篩選**（顯示說明文字）
  - 名稱輸入框使用 `.form-input`（`box-sizing:border-box; width:100%`，左右對稱）
- **句段集工具列批次動作**
  - 指派：重用 `fileAssignModal` + `_fileAssignModeIsView` 旗標，路由至 `DBService.assignView/unassignView`
  - 移除：確認後批次 `DBService.deleteView`
  - 分析：透過 `_wordCountSegmentOverride` 旗標傳入 segmentIds 覆蓋 file 模式的 `runWordCountAnalysis`
  - 拆分：透過 `_splitSegmentOverride` 旗標傳入 segmentIds，新函式 `runSplitHintWeightedPreviewBySegIds` 計算
- **句段集編輯器**（`openEditorFromView` → `openEditorWithSegments`）
  - 進入時顯示說明 Modal（`viewEditorIntroModal`）
  - 表格新增「所屬檔案」欄（`col-source-file`）
  - 禁用 mqxliff 身分選擇與匯出按鈕

### 已知限制

- 句段集的句段在建立時**快照凍結**（`segment_ids` 陣列），不會隨原始檔後續匯入自動更新
- 自訂篩選目前只能「收錄全部檔案句段 + 自訂名稱」，進階篩選（狀態、標籤…）尚未實作
- 如以舊版本（有 1000 筆截斷 bug）建立的句段集，**必須刪除並重建**才能取得完整行數
...
| 項目                                             | 規格位置                     |
| ---------------------------------------------- | ------------------------ |
| §5.2 自訂篩選精靈 Step 2（搜尋工具列 + 唯讀格線預覽）             | `CAT_VIEW_SPEC.md §5.2`  |
| §4 句段集清單「整體進度」欄                                | `CAT_VIEW_SPEC.md §4`    |
| §12.1 Notes 面板 2 張資訊卡（上方：所屬句段集；下方：跨句段集重複率）     | `CAT_VIEW_SPEC.md §12.1` |
| §12.3 編輯器工具列「顯示所屬檔案」checkbox                   | `CAT_VIEW_SPEC.md §12.3` |
| §3.1 未指派讀者唯讀 + 系統訊息                            | `CAT_VIEW_SPEC.md §3.1`  |
| §13.3 儀表板「我的指派」顯示句段集指派                         | `CAT_VIEW_SPEC.md §13.3` |
| §14 協作房間支援（view room）                          | `CAT_VIEW_SPEC.md §14`   |
| 離線版（Dexie `views` 表已建，RPC stub 已加，UI 尚未串接離線路徑） | `cat-tool/db.js`         |


### 踩坑與維運重點

- `getSegmentsByFileForPreview` 的雙重分頁迴圈是關鍵：外層控制 file_id IN () 的批次大小（20），內層 `.range()` 每次取 1000 筆直到空回傳，兩層缺一都會截斷
- `_fileAssignModeIsView` 旗標控制 `fileAssignModal` 的儲存路由，關閉 Modal 時（`closeFileAssignModal`）必須重置為 `false`，否則會污染後續的檔案指派
- `_wordCountSegmentOverride` / `_splitSegmentOverride` 在 handler 讀取後**立刻清空**（`= null`），防止後續觸發時帶入過期的 override

## CAT：全專案 UI 準則（2026-05 建立）

見 `[.cursor/rules/ui-conventions.mdc](../.cursor/rules/ui-conventions.mdc)`，正式列為 Cursor 規則（`alwaysApply: true`）：

1. **表單輸入框對齊**：`.form-input` 統一加 `box-sizing: border-box; width: 100%`，Modal 外層 `<div>` 左右 padding 必須對稱（預設 `1.25rem`）
2. **時間顯示 24 小時制**：所有 `toLocaleString` 必須帶 `{ hour12: false }`；React/TS 側優先使用 `src/lib/format-timestamp.ts` 的工具函式

## CAT：內聯標籤點擊插入、Alt+S（2026-05）

- **點擊原文欄 tag 插入同列譯文**（假游標位置或句末、重複佔位不插、鎖定／協作保護）：行為與實作位置見 `[cat-tool/README.md](cat-tool/README.md)`「編輯器：內聯標籤」；`[docs/CODEMAP.md](CODEMAP.md)` 亦有對照列。
- **Alt + S 標籤展開／收起**：注音等鍵盤下改以實體 `KeyS` 偵測，說明見同上 README。`[cat-tool/index.html](../cat-tool/index.html)` 快捷鍵 modal 已補「點擊原文標籤」「實體 S 鍵」說明；改後須 `npm run sync:cat`。
...
- **譯者個人覆寫**：僅存 `**localStorage`**，鍵名 `catQfColOverride_{projectId}`；不覆寫管理者存進專案的預設。首次開啟欄位設定（尚無有效 LS 內容）時會先向 DB 取最新專案列，以**管理者當下預設**填入並寫入 LS 快照，故不需按「儲存」也會沿用該組對應；之後譯者若另存則以 LS 為準。開啟對話框與「提問表單」複製前皆會 `**getProject` 刷新**，避免編輯器閉包內的舊 `project` 造成畫面與剪貼簿不一致。
...
| Commit    | 摘要                                                                                                                                                                                                            |
| --------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `（文件）`    | [CAT_BATCH_IMPORT_WIZARD_SESSION.md](./CAT_BATCH_IMPORT_WIZARD_SESSION.md)：驗收完成收尾（§五～§六 狀態、§七 條列、§九 提交對照表、§十 修訂；第一節第 8 點改「未採納」）；對應 SHA 以 `git log -1 --oneline -- docs/CAT_BATCH_IMPORT_WIZARD_SESSION.md` 為準 |
