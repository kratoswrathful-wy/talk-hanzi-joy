...
| Commit | 摘要 |
|--------|------|
| `d49b89e` | CAT [`style.css`](../cat-tool/style.css)：`.primary-btn` 等 `:disabled` 反灰與 `cursor: not-allowed`；批次「開始匯入」等鎖定態可辨識；說明見 [CAT_BATCH_IMPORT_WIZARD_SESSION.md](./CAT_BATCH_IMPORT_WIZARD_SESSION.md) 5.6 |
