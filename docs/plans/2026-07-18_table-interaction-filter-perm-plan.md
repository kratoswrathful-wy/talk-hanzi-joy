...
### 0.4 審核結論與驗收門檻收斂（2026-09-07）

**本項程式審核通過，可提出小修發布核准；尚未核准合併／部署。** 審核基準 `262e5dc8634b131974e1f78ea4432ca0530ccd6a`。

- Codex 已檢查相對 `bbfa5cb3` 的程式差異：只調整 CasesPage 選項來源及新增對照 helper；提交鍵、保存邏輯、RPC、資料庫與權限均未改動。
- Codex 本機重跑 helper 2 項與 CasesPage 接線／元件 mock 5 項：**7/7 通過**。接線檢查屬原始碼斷言，互動屬元件 mock，不宣稱完整頁面或真實後端驗收。
- GitHub Playwright run `34089473750` 已完成，`success`，HEAD 為上述 SHA；這是既有 E2E 結果，不代表補做了本修真實保存測試。
- §0.1／§0.3 記載的真實保存／重新整理讀回仍為**未執行**。經此小修的實際差異審核，該項不再單獨作為本 PR 阻擋，不要求為此新建隔離庫、付費資源或修改正式案件。這是明確調整驗收範圍，並非將未跑改記為通過。
- 後續若改到保存／權限／RPC，須重新評估相應整合測試，不能套用本次豁免。
- 如使用者另核准發布，開站後只需在實際部署版本確認選單可開、選項與既有值正確；不為驗收強制寫入真實案件。本輪不要求維護頁、停用 API、重跑 Gate1 或再次 Slack 操作。
- 本次文件變更不等於操作授權。Cursor 更新同一 PR 狀態並提出一次精簡發布方案，明列目標 deployment／SHA、與已部署版本差異，以及 main 尚未收斂時的處置；使用者核准前不 merge／promote。main 的後續收斂仍需追蹤，但不因此重做本項功能。
