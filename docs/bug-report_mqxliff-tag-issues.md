...
> 更新：2026（階 1–3：CSS 緩衝、零 tag 匯出、譯文 mq: 補 target_tags、**階 3 搜尋不覆寫原文 pill**、`normalizeXmlForSig` 加強、副檔名 `.mxliff` 說明）
...
| **Bug #4 搜尋** | 已實作 | 篩選/高亮前以 `buildTaggedHtml` 重畫原文 `.rt-editor` 並 `updateTagColors`；在 `.rt-editor` 內依與內文一致的索引只包文字／換行、不跨 tag 佔位 | [`cat-tool/app.js`](../cat-tool/app.js) `runSearchAndFilter`、`getSourceTextSegmentsForHighlightMap`、`applySourceSearchHighlightsInRtEditor` |
...
5. **階 3 / 搜尋**：原文含 `{1}` 等 tag pill 時，在搜尋/篩選觸發高亮後，pill 仍應**完整**、字串不應變成一段扁平 HTML；`normalizeXmlForSig` 不影響你手動比對的步驟，屬內部標籤成對用。
