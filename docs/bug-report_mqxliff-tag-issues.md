...
| **成對 tag 箭頭外框／斜角缺框** | **已修** | `drop-shadow` 沿 `clip-path` 輪廓描邊 + `--tag-fill` 底色（取代雙層 pseudo，修正斜角／橫邊粗細不均）。方案 B | [`style.css`](../cat-tool/style.css)；專文 [`CAT_PAIRED_TAG_ARROW_BORDER_IMPLEMENTATION_PLAN.md`](CAT_PAIRED_TAG_ARROW_BORDER_IMPLEMENTATION_PLAN.md) |
