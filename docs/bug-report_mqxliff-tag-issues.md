...
| **Bug #12 mq:rxt val 屬性不符（reconcile 短路）** | **已修並驗收**（`2a88a48`） | 原文 ph `val="{hours:int}"` 與 TM 帶入 `val="{value:int}"` — `innerEscapedTagSig` 只看元素名稱短路 → reconcile 不觸發、F8 不補、QA 不報。修正：fall-through + normalize 移除 id + QA val 比對；樣本 `36432` 列 203 產品驗收通過 | [`xliff-tag-pipeline.js`](../cat-tool/js/xliff-tag-pipeline.js)、[`app.js`](../cat-tool/app.js)；專文 [`bug-report_mqxliff-mq-rxt-val-mismatch_2026-06.md`](bug-report_mqxliff-mq-rxt-val-mismatch_2026-06.md) |
