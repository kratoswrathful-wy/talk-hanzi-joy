...
| **成對 tag 箭頭外框／斜角缺框** | **已修** | 雙層 pseudo（`::before` 外框色、`::after` `--tag-fill` 內填）取代 `border`+`clip-path`；斜角尖端描邊完整。方案 B | [`style.css`](../cat-tool/style.css)；專文 [`CAT_PAIRED_TAG_ARROW_BORDER_IMPLEMENTATION_PLAN.md`](CAT_PAIRED_TAG_ARROW_BORDER_IMPLEMENTATION_PLAN.md) |
