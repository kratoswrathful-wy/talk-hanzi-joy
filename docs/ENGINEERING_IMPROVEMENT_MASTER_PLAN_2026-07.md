狀態：實作中（階段一二已落地；W10、W9-A、W9 wave 2 為擁有者插隊裁定工項，已落地並多半驗收；階段三——lint 清零批次 1–8、Vitest 批次 2、`dev-switch-user`、`R2` 五關均已驗收併入 main（殘留 OBS-5 待辦）；**e2e 接入 CI 已驗收併入 main**（`2a69e7c5`，見 §20）；`cat-cloud-rpc.ts` 297 處 `any` 另立獨立工項（§15）；階段四未開工）
...
- **e2e 接入 CI** — 狀態：**已驗收**（獨立 `.github/workflows/e2e.yml`；非擋關：workflow_dispatch + nightly + pull_request；強制測試模式；secret 缺則 skip；失敗上傳 artifact；過程修正 `VITE_SUPABASE_URL` secret 誤設、兩個既有 spec 首次於 CI 曝光的 bug；見 §20）— commit：`2a69e7c5`
...
- 關聯案件僅顯示標題與連結；點入案件後的欄位可見性依案件管理矩陣（工項 D／§9.2 案件段）。
...
**分級制**：本修正單因抽驗失守走慢軌一次；結案後 lint 類批次恢復快軌資格。**已驗收併入 main**（merge `e2e78d3e`，2026-07-06）。
...
**已驗收**（2026-07-07，`main` @ `2a69e7c5`）：PR #35 的 `pull_request` 觸發自動跑出 E2E workflow（非擋關四護欄皆驗證有效），過程中排查並修正兩個此前從未在 CI 環境真的執行過而首次暴露的既有問題（均已同分支修正，非本工單新增行為變更）：

1. **GitHub Secret `VITE_SUPABASE_URL` 設定錯誤**：值誤設為正式站網域 `https://talk-hanzi-joy.vercel.app`（`vercel.json` 只有 SPA 全轉址、無 `/auth/v1/*` 代理），導致本機開發伺服器送出的登入請求打不到真正的 Supabase Auth，90 秒逾時、零筆登入紀錄。已用 `gh secret set` 改回正確的 `https://wshsmerltcakffllgyul.supabase.co`（Supabase MCP `get_project_url` 查證）。
2. **`tests/cat-navigation-2-3q.spec.ts` 模組層級直接讀大檔 fixture**：CI 沒有機器本機的 `Test_Big.mqxliff`，直接拋錯讓整個 `playwright test` 行程 exit code 1、零測試執行。改為 try/catch 取得 fixture，缺席時「Test A–I — 大檔」整組以 `test.skip` 優雅跳過。
3. **`tests/ai-bridge-phase2.spec.ts` P2-L7 讀錯欄位路徑**：`clientInvoice.create` 回傳的 id 巢狀在 `data.invoice.id`，該測試誤讀 `data.id`（undefined），導致 `get()` 一律查無此筆。改讀 `data.invoice.id`（與同目錄 `lms-client-invoice-bridge.spec.ts` 寫法一致）。

最終 PR #35 merge 前：E2E workflow 45 項全過（11m12s）、五關 CI 綠燈、Vercel 部署綠燈。
