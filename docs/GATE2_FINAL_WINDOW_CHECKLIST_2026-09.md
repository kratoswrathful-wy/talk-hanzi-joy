...
| 項目 | 值 |
|---|---|
| Gate1 已驗證基準（17 支） | 見既有 dry-run（最高 `20260904004224`） |
| **本工項新增** | `20260905120000_gate2_maintenance_write_acl.sql`（第 **18** 支） |
| 正式開窗 dry-run | 必須重跑；預期 **18**（或當時實際 pending 數）；**禁止**硬寫死「仍為 17」 |
| Gate1 覆蓋 | **不含**第 18 支；須本機／隔離定向測通過後才可納入整包 |
