...
| 修復 | 單鍵顯示時加 `.split-btn-solo`（全圓角 + `var(--border-color)` 右框）；`refreshWfTaskCompleteToolbar` 依箭頭可見性切換 |
| 狀態 | **本輪實作**（待驗收） |
...
| 2026-06-24 | **`61e8fc2`**：Team 模式 `addSegments` 補寫 `wf_trans_confirmed_at/by` 等，勾「在系統內標為確認」重開檔可顯示實心綠點。匯入進度疊層、TM 去重見 [`bug-report_import-confirmed-tm-write-progress-overlay_2026-06.md`](./bug-report_import-confirmed-tm-write-progress-overlay_2026-06.md)。 |
| 2026-06-24 | 附錄 A：準備完成按鈕 `split-btn-solo` 右框線修復 |
