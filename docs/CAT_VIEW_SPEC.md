...
使用者可見之「禁止編輯」tooltip 或同類提示**一律以「禁止編輯」起首**：

- 短原因：`禁止編輯，{原因}`（例：`禁止編輯，檔案準備中`）
- 較長說明：`禁止編輯：{原因}`（例：`禁止編輯：匯入時即已鎖定`）

完整情境與 Workflow 確認狀態 UX 見 [`CAT_WORKFLOW_CONFIRM_STATUS_UX_2026-06.md`](./CAT_WORKFLOW_CONFIRM_STATUS_UX_2026-06.md) §10。
...
- **相對欄序（固定）**：句段集模式中，`col-source-file` **必須**緊接在所有 Key 欄（`col-key-*`）之後、原文欄（`col-source`）之前；不因使用者曾於 ⚙️ 欄位設定拖曳排序或舊版 `catToolColSettings` 順序而改變。實作於載入欄位設定後呼叫 `ensureSourceFileColAfterKeys()`。
- **狀態欄**：仍固定於整列最右（於 `colSettings` 陣列末端）；實作：`ensureStatusColumnLast()`。
...
- **適用範圍**：**單檔編輯器**與**句段集編輯器**之主格線表頭皆支援欄際拖曳調整可調欄之寬度；調整後寫入 `colSettings[].width`，並以 `localStorage` 鍵 **`catToolColSettings`** 持久化（與既有欄位顯示設定並存）。
- **固定欄（不提供拖曳把手／寬度由版面強制）**：`col-repetition`（重複）、`col-match`（相符度）、`col-status`（狀態）—即格線最右三欄。
- **實作捷徑**：[`cat-tool/app.js`](../cat-tool/app.js) `attachColResizer()`、`applyColSettings()`。

### 12.5 Key／額外資訊：全空白自動隱藏

- **適用欄位**：各 **`col-key-*`** 與 **`col-extra`**（額外資訊）。
- **判定**：以目前載入之句段清單為準；該欄在**每一句段**皆視為空白時觸發：
  - Key：對應索引之 `seg.keys[i]` 缺失，或 trim 後為空字串。
  - 額外資訊：`seg.extraValue` 缺失，或 trim 後為空字串（與格線儲存格渲染所用欄位一致）。
- **行為**：自動將該欄 `visible` 設為 `false`，並於記憶體標記 `autoHidden`。**不**自 `catToolColSettings` JSON 還原 `autoHidden`；每次開啟檔案或句段集時重新掃描並更新。
- **⚙️ 欄位設定（檢視設定）**：若 `autoHidden` 為真，該列 checkbox **disabled**、列不可拖曳排序，並顯示說明：此欄在目前檔案／句段集中完全空白，故自動隱藏；待資料於後續載入中有非空白內容時，下次開啟即可再度顯示（並解除鎖定）。
- **作業捷徑**：[`cat-tool/app.js`](../cat-tool/app.js) `applyAutoHiddenCols()`、`renderColSettings()`。
