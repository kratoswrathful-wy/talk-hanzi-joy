...
|------|------|
| **名稱** | `name` |
| **涉及檔案** | 多行文字，每行 `#<序號> <檔名>`；檔名過長**截斷**，`title`／tooltip 顯示**完整檔名**；**全部列出**（不折疊為「+N 個」） |
...
- 產生邏輯應與編輯器篩選一致，可重用或對齊 [`cat-tool/app.js`](../cat-tool/app.js) 之 `getSfFilterGroupConditionLines`（約 L10382 起）的**用語**（例如「句段狀態：…」「翻譯記憶相符度：…」）。
...
2. **步驟二（預覽）**：介面類似句段編輯器格線，**唯讀**——顯示來源／譯文與狀態等欄位，**不提供可編輯譯文區**；篩選條件變更時**即時**更新預覽列。
...
| **匯出檔案** | **不顯示**匯出按鈕（跨多檔、多格式，避免誤導單檔匯出） |
...
### 12.1 格線與筆記面板之間的兩張資訊卡

- **插入位置**：[`cat-tool/index.html`](../cat-tool/index.html) 中 `#editorGrid`（`grid-viewport`）與 `#notesPanel` 之間，或 `#notesPanel` 內、`#notesPanelResizer` 上方。
- **顯示條件**：僅句段集模式；單檔模式隱藏整個容器。
- **內容**（兩張卡片，文案範例）：
...
- **標題**：句段集模式下，[`cat-tool/index.html`](../cat-tool/index.html) 之 `#editorFileName`（`span.editor-file-title`）顯示為「句段集：（句段集名稱）」，而非單一檔名。
- **勾選方塊**：置於 `toolbar-left`（例如 `#editorFileName` 右側），文案「檢視句段所屬檔案」。
