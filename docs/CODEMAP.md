...
| **譯文換行編輯補修**（tag 旁刪字勿誤存 `\n`、¶ 模式可刪語意換行） | [`cat-tool/app.js`](../cat-tool/app.js)：`isGhostBr`、`tryDeleteSemanticNewlineAtCaret`（Wave 1）；`applyNonPrintMarkers`／`canonicalizeTargetEditorFromExtractPlain`／`isGhostOnlyDiv`（Wave 2 Fix 3A／3C／3B）；計畫 [`CAT_TARGET_NEWLINE_EDIT_NP_PLAN.md`](./CAT_TARGET_NEWLINE_EDIT_NP_PLAN.md) |
