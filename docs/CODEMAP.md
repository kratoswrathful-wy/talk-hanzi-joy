...
| **Excel 匯入警語 + 括號／regex 轉 tag**（規劃中；先 Rich Text 再字串規則、`_batchExcelConfigs`／`wizardStepBatchExcel` 等掛載點） | [EXCEL_IMPORT_TAGS_SPEC.md](./EXCEL_IMPORT_TAGS_SPEC.md)（工程規格）；摘要與驗收條列 [EXCEL_IMPORT_TAG_WARNING_PLAN.md](./EXCEL_IMPORT_TAG_WARNING_PLAN.md)；實作掛載 [`cat-tool/app.js`](../cat-tool/app.js) `wizardStepBatchExcel`、`showBatchExcelConfigModal`、`_batchExcelConfigs`；富文字 [`cat-tool/js/xlsx-rich-tags.js`](../cat-tool/js/xlsx-rich-tags.js) `extractCellRichTags` |
