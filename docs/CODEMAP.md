...
| **XLIFF → TM 匯入**（`.xliff`、`.xlf`、`.mqxliff`、`.sdlxliff`；選檔後 `#tmXliffImportDialog` 與編輯器同款篩選；`CatToolXliffBuildSegments` → `buildTmImportCandidates` → `evaluateSegment`／`segmentPassesSfRowRangePure` → `bulkAddTMSegments`） | [`cat-tool/js/xliff-build-segments.js`](../cat-tool/js/xliff-build-segments.js)、[`cat-tool/js/xliff-to-tm.js`](../cat-tool/js/xliff-to-tm.js)；[`cat-tool/app.js`](../cat-tool/app.js) `runTmXliffFilteredImport`；[`cat-tool/index.html`](../cat-tool/index.html) `#tmXliffImportDialog`；[CAT_XLIFF_TO_TM_IMPORT_PLAN.md](./CAT_XLIFF_TO_TM_IMPORT_PLAN.md) |
...
| **QA「Tag 檢查」**（與匯出／譯文 `{N}` 佔位對齊；`runQaChecks`、`_qaPushSegmentRuleFindings`、`_qaTagIdForCompare`、`_qaPlainTargetTagNumSet`） | [`cat-tool/app.js`](../cat-tool/app.js)；說明與誤報根因見 [`bug-report_cat-qa-tag-parity.md`](./bug-report_cat-qa-tag-parity.md)；介面折疊（檢查範圍/右下角資訊區）需求與驗收紀錄見 [CAT_QA_UI_COLLAPSE_HISTORY_2026-05.md](./CAT_QA_UI_COLLAPSE_HISTORY_2026-05.md) |
