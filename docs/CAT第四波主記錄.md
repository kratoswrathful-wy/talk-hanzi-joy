...
| **H2** | **Ctrl+↑／↓**：在檔案編輯檢視開啟時，**上一可見句／下一可見句**（篩選下略過隱藏列），**焦點與 caret 在該句譯文開頭**（文件層攔截，**不限**譯文格內焦點）。**Alt+↑／↓**：**CAT** 分頁比對表或 **TM 搜尋**分頁 concordance 列表有結果時，**僅移動選取**（不套用譯文），**取代** G7 以 Ctrl 捲動 CAT 列。 **Ctrl+Alt+←／→**：**右欄分頁**切換。**Ctrl+Alt+↑**捲到暫存游標列並顯示假游標；**↓** 依序還原至譯文或顯示假游標（文件層、不限焦點）。**清除譯文**：**Ctrl+Shift+Insert**。**假游標**限制在 **#editorGrid** 內、捲出可視時於邊界顯示「第 xx 號句段」提示。 |
