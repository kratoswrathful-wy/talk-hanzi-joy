...
- **手動 smoke（建議）**
  - **5-g、§5-h、§5-i**：若 **5-g** 與 **5-h** 對快捷鍵／行為敘述**衝突**，以 **§五 5-h** 為準。**Ctrl+1～9** 之**細部套用規則**（真／假游標、焦點在搜尋列、TB／Frg 與 TM 差異等）若與 **5-g G4** 牴觸，以 **§五 5-i** 與 [`docs/CAT_CTRL_DIGIT_SHORTCUT_SPEC.md`](CAT_CTRL_DIGIT_SHORTCUT_SPEC.md) 為準；**5-h** 仍優先於與 **G7** 牴觸之敘述（維持既有）。
  - **A**：TM 單擊不貼上、雙擊貼上（受「雙擊套用譯文」開關約束，見 **§五 5-f**）；**Ctrl+1…9** 依右欄**作用中分頁**套用 **CAT 或 TM** 列表（**5-g G4**；**細部行為**見 **§5-i**／專用文件）；**Ctrl+K**：譯文欄反白後搜尋結束游標／選取須與觸發前一致；非譯文欄反白後搜尋結束焦點在譯文**開頭**；原文欄反白時 TM 搜尋欄位為 **原文**、譯文欄為 **譯文**；**Ctrl+F** 由編輯區帶選字時寫入篩選搜尋列；**#tmSearchField** 變更且已有關鍵字時自動重搜；假游標與 Ctrl+0；**句段列狀態色全欄一致**（**5-f D1**）；**新增術語成功或 Enter 後回譯文游標**（**5-f D2**）；**§5-h** 下快捷鍵 modal、按鈕 `title` 與實際綁定一致；右欄套用／插入之 **Z／Y** 與相符度欄一致（**5-h H4**）。
...
| 2026-05-02 | （待實作後補） | **5-i**：Ctrl+1～9 細部規格文件化；見 **§五 5-i** 與 [`docs/CAT_CTRL_DIGIT_SHORTCUT_SPEC.md`](CAT_CTRL_DIGIT_SHORTCUT_SPEC.md) |
...
### 5-i（後續決策）：Ctrl+1～9 細部行為

> **完整條文**見 [`docs/CAT_CTRL_DIGIT_SHORTCUT_SPEC.md`](CAT_CTRL_DIGIT_SHORTCUT_SPEC.md)。本節為摘要；與 **§5-g G4** 關係：**列表來源**（TM 搜尋分頁 vs CAT 分頁）仍依 **G4**；**寫入哪一句、是否整段取代、焦點在搜尋列時是否無效**等以本節與專用文件為準。

| 項目 | 規格摘要 |
|------|----------|
| **TM vs TB／Frg** | **TM**：整段取代目標譯文格。**TB**（術語庫條目）、**Frg**（片段相符，CAT 列表簡稱）：僅於游標邏輯位置**插入**建議譯文，不整欄取代。程式 `type` 與 Frg 之對應見專用文件 §2。 |
| **目標列與位置** | 以**真游標**或**假游標／暫存游標**為準（**不**僅依 `active-row`）；見專用文件 §4。 |
| **焦點在譯文格** | 同專用文件 §5.1：TM 整段取代；TB／Frg 插入後游標在插入內容之後。 |
| **焦點在 `#sfInput`／`#sfReplaceInput`** | **TM**：不套用，於游標附近短暫提示「目前游標在搜尋工具列，無法插入 TM 比對／搜尋結果」。**TB／Frg**：**純文字**插入該輸入框游標處。 |
| **焦點在其他處** | 對齊譯文格邏輯，但以假游標定位；完成後真游標回到該列譯文格對應位置（專用文件 §5.3）。 |
| **失敗** | 目的列隱藏、無法編輯或無游標記錄時：不寫入，顯示錯誤說明（專用文件 §6）。 |
| **Undo／相符度** | 仍須符合 **§5-g G3**、**§5-h H4**（專用文件 §7）。 |

**程式預定修改點（5-i）**：[`cat-tool/app.js`](../cat-tool/app.js)（`applyNumpadMatchForActiveTab`、`handleCatResultApply`、假游標與搜尋列焦點判斷等）、[`cat-tool/index.html`](../cat-tool/index.html)（工具提示／文案占位若需）；實作後 **`npm run sync:cat`** 並補 **§5-e** 列 commit。
...
## 七、本聊天室收斂紀錄（提問表單／案件綁定／註記／協作）

> 本節記錄本聊天室連續實作與驗收收斂結果，作為 CAT 維運與稽核基準。  
> 關聯 commit 範圍（功能主線）：`0c641ca` → `6c628fb`（含其間修正批次）。

### 七點一、提問表單與 LMS 案件綁定（初版）

- 新增專案層級欄位 **`外部提問表單連結`** 與檔案層級 **`LMS 案件`** 綁定流程，編輯器工具列依條件顯示 **`提問表單`**、**`內部註記`**。
- `AI 輔助` 按鈕維持原有規則，不受「提問表單／內部註記」條件連動隱藏影響。
- 清理已廢棄 `#btnTagGroupMode`（`T⁺`）及對應事件路徑，避免誤導為仍可用功能。

### 七點二、UX 修正（文案、版位、按鈕與互動）

- 專案頁文案統一為 **`外部提問表單連結`**。
- 儲存後採 **inline 顯示**：網址原地保留且可直接點擊，不再額外在區塊下方重複顯示。
- 檔案清單語言對欄位調整為兩行排版（目標語言移到第二行）。
- 指派欄位調整為一人一行顯示，並將欄位內容垂直置中。
- 新增 **`連結案件`** 批次按鈕（套用至勾選檔案），與原有單檔流程共存。

### 七點三、穩定性修正（重整後遺失、游標殘留）

- 修正外部提問表單與檔案綁定案件在重整後遺失問題，資料改為依專案/檔案持久化來源重新載入。
- 新增離開編輯器時的強制清理流程，統一移除假游標與真假游標捲動提示，避免殘留 UI artifact。
- 補強條件渲染時序，避免「已設定但按鈕未顯示」的狀態落差。

### 七點四、行為擴充（批次連結、批次指派、狀態驅動同步）

- **批次連結案件**：可對多個勾選檔案一次套用 LMS 案件關聯。
- **批次指派**：指派對話框支援多檔案同時指派，不再限制單一檔案。
...
- 由編輯器建立註記時，自動預填：
  - `檔案名稱`：目前編輯檔案完整名稱
  - `ID / 行數`：目前句段 ID（或對應行資訊）
  - `問題或註記內容`：帶入編輯器選字（有選取時）
- 開啟註記頁後，自動聚焦 `問題或註記內容`，縮短人工點擊路徑。

### 七點六、驗收覆蓋（本聊天室最終）

- 重整後：`外部提問表單連結` 與 `LMS 案件` 綁定皆不遺失。
- 批次行為：`連結案件`、`指派` 皆可對多個勾選檔案生效。
- 註記命名：新建命名符合 `案件標題_Note_00001` 類型格式。
- 註記預填與焦點：由編輯器建立註記可自動帶入欄位並聚焦內容欄。
- 狀態驅動同步：`詢案中 -> 已派出` 轉換時，可把案件頁譯者/審稿人員新增到連結檔案指派名單。
...
### 八點七、建立句段集：預覽選項與分階段實作（文件／待實作）

- **索引**：[`docs/CAT_VIEW_CREATE_ROADMAP.md`](CAT_VIEW_CREATE_ROADMAP.md)（句段集建立／預覽之**產品決策**、**分階段實作順序**；已落地之清單 UX `7895783` 僅交叉引用 **§八點六**）。
- **狀態**：本小節**尚無對應單一 commit**（決策與階段表僅文件）；待第一階段程式落地後再補 commit 與驗收。

---

## 九、搜尋取代增強、Ctrl+1～9 實作與雜項修正（2026-05）

> 本節記錄第四波結案後之 CAT 編輯器增強與本輪診斷事項，**不更動**上文第四波 A/B 結案判定。

### 九點一、`c4ef6fe`：Ctrl+1～9 細部行為（§5-i）+ F4 篩選快照初版修正
...
| 焦點情境 | TM | TB／Frg |
|----------|-----|---------|
| 譯文編輯區 | 整段取代；游標在插入後 | 於真游標處插入；游標在片段後 |
| 搜尋／取代輸入框 | 不套用，短提示 | 純文字插入該框游標處 |
| 其他（假游標） | 對齊譯文格整段取代邏輯 | 對齊插入邏輯；完成後還原真游標 |

**同步**：`cat-tool` 變更經 `npm run sync:cat` 一併提交 [`public/cat/`](../public/cat/)。

### 九點二、`68170cd`：CAT 工具列與進入模式整理

| 項目 | 說明 |
|------|------|
| **single-row progress reload** | 句段集／檔案清單進度欄可點擊重載**單列**進度，無需整表刷新。 |
| **mixed toolbar unify** | 句段集與單檔共用工具列之混合情境收斂（版位／按鈕一致）。 |
| **instant tooltips** | 工具提示延遲縮短，滑過即顯。 |
| **reset modes on entry** | 進入編輯器時重設搜尋／篩選相關 UI 狀態，避免帶入上一檔殘留。 |

### 九點三、`4437467`：搜尋取代第二批增強與 Backspace 修正
