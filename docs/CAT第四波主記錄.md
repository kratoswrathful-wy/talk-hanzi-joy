...
### 5-d（待辦）：重製線上 TB 匯入（第四點五波收尾）

| 項目 | 說明 |
|------|------|
| **主計畫位置** | 第 **10** 節 **§10.3 重製線上 TB 匯入**（主計畫鏡像；**細項與本節對齊**）；todo **`tb-online-import-column-mapping`** |
| **目標（摘要）** | 與 Excel 匯入相同之**可自選欄位映射**（來源／目標／備註／比對旗標等）、預覽首批列、成功／失敗摘要；`matchFlags` 與 §10.1／10.2 一致 |
