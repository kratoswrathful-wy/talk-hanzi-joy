...
**狀態標記（2026-04-26）**：第四波進行中（A 初版 `ebb9ee4` 已驗收；A 驗收修正 `8f8cea8` 三項已通過使用者複驗；**追加修正 `d326666` 已實作**；**篩選快取 `sfRowRenderCache` 根因已確認、程式修正待實作**；B 待開工）。
...
|------|------------|------------------|------|
| **A** | §11 | `live-tm-cursor-ux` | **初版 `ebb9ee4` 已驗收**；**`8f8cea8`**：(1) `Ctrl+0` 納入 undo；(2) 譯文欄 `Ctrl+K` 範圍；(3) `Ctrl+0` 游標；**`d326666`**：`Ctrl+Y` redo 方向修正、篩選下確認後跳可見未確認句段、`Ctrl+↑↓` 跳過隱藏列、批次確認後補 `runSearchAndFilter()`（篩選視覺仍有一項待 `sfRowRenderCache` 修正） |
...
- **追加修正**（`d326666`）：(1) `applyEditorRedo` 對 redo 堆疊內已交換 old/new 的 mirror entry 改傳 `applyOneTargetUndo(..., 'undo')`，重做才會套到正確譯文；(2) `getAfterConfirmFocusIndex` 在篩選模式下略過 `display:none` 列；(3) 譯文欄 `Ctrl+↑↓` 以 `while` 跳過不可見鄰列；(4) 批次確認／`confirmOp` undo-redo 路徑在 `renderEditorSegments()` 後補 `runSearchAndFilter()`。**使用者複驗**：(1)～(3) 通過；(4)(5) 仍發生。
- **待修（根因已確認）**：`renderEditorSegments()` 重建 DOM 後，[`cat-tool/app.js`](cat-tool/app.js) 內 `sfRowRenderCache` 仍保留舊的 `vis`；`runSearchAndFilter()` 以 `rowCache.vis !== vis` 才更新 `row.style.display`，快取與新 DOM 不一致時會跳過更新，導致應隱藏列維持可見，且後續變更篩選條件時 `vis` 仍相等而永遠不更新。**建議修法**：在 `renderEditorSegments()` 開頭 `sfRowRenderCache.clear()`（或等價重置 visibility 快取）。尚未實作。
...
- **複驗紀錄（2026-04-25）**
  ...
  - **尚餘一項**：篩選下多選批次操作（確認／右鍵設為未確認等）後，隱藏列仍全部顯示且篩選條件變更失效 — **未通過**；根因見完成清單「`sfRowRenderCache`」條，待程式修正後再驗。
- **驗收結論**：第四波 A 初版已通過使用者驗收；`8f8cea8` 三項已複驗通過；`d326666` 為第二輪問題之修正批次，**4/5 複驗通過**，篩選快取一項待修；第四波 B 待開工。
