...
| 日期 TBD | commit TBD | 5-h：驗收回饋（租約自鎖、快捷鍵、tooltips、右欄 undo+相符度）；見 **§五 5-h** |
...
| **H2** | **Ctrl+↑／↓**（主編輯區、可編譯格內）：**上一可見句／下一可見句**（篩選下略過隱藏列），**焦點與 caret 在該句譯文開頭**。**Alt+↑／↓**：右側 **CAT** 分頁有比對表且條件符合時，**比對列表選取**上／下（**取代** G7 以 Ctrl 驅動 CAT 列之設計）。**Ctrl+Alt+←／→**：**右欄分頁**切換（取代 5-g 中 **Ctrl+←／→** 換分頁之實作）。**Ctrl+Alt+↓**：焦點至**假游標**（`catSavedCaret`／`showCatFakeCaret` 與產品既有語意一致）。**清除譯文**：**Ctrl+Shift+Insert**（**取代** Ctrl+Delete 綁定）。 |
| **H3** | **單一真相**（常數或 registry）：按鈕 **`title`、快捷鍵 modal 固定列** 與 **實際 keydown** 顯示字串一致，含「複製原文到譯文」「清除譯文」等；避免只改程式、漏改 HTML 靜態字。 |
| **H4** | 右欄**插入／套用**（不僅 **F8**）凡變更譯文者，**Ctrl+Z／Y** 須併**相符度**欄套色與數字；**禁止**因 `oldTarget === newTarget` 等**早退**導致僅變更相符度時無法還原。 |
