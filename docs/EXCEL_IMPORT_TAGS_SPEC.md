...
2. **字串層規則**（本規格新增）：在步驟 1 得到的 `text` 上，依使用者勾選依序套用：
   - 內建括號（順序建議：`<>` → `[]` → `{}` → 字面 `\n`，或產品另定**固定順序**並寫死於文件與測試）；
   - 再套用**自訂 regex 列**（由上而下，或產品定序）。
3. 每觸發一段「需包成 tag」之子字串，產生新的 **open/close** tag 物件，**`{n}`、`{/n}` 之數字 n 必須接續**既有 `tags` 中已使用之編號（含步驟 1 已產生者）。
4. **`tag.xml`**（或 Excel 富文字還原所需欄位）之填充方式須與 [`buildRichTextXml`](../cat-tool/js/xlsx-rich-tags.js) 之假設相容— **具體 XML 片段**由實作與產品共同定案（本規格只要求「不得破壞既有匯出假設」）。
...
- **`<>`**：最短合理定義為「由 `<` 至下一個 `>` 之區段」或「不含巢狀之成對」— **須實作時選一並補單元測試**（含未閉合、巢狀 HTML 碎裂等）。
- **`[]`、`{}`**：同理；**`{}` 須避免**與已插入之 `{1}`、`{/1}` **重疊誤套**— 規則：僅在「**尚未標記化之純文字區間**」套用，或對「使用者來源字串」只做一輪掃描後再合併 tag（實作擇一並文件化）。

### 6.2 字面 `\n`

- 比對連續兩字元：**`\`**（U+005C）+ **`n`**（U+006E）。
- **不**與儲存格內 **Unicode 換行**（例如 U+000A）混淆。

### 6.3 嵌套

- MVP 建議：**不支援**跨層嵌套之複雜括號，或採「**由內向外**／**最長匹配**」之單一明確策略；並在 UI 以一句話註記極端案例可能略過。
