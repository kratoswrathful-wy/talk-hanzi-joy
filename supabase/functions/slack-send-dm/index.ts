...
import { rejectWhenMaintenanceEnabled } from "../_shared/maintenance-gate.ts";
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const maintBlocked = await rejectWhenMaintenanceEnabled(jwt);
    if (maintBlocked) return maintBlocked;
