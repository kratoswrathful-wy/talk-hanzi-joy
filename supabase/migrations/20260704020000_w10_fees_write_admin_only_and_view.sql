-- W10 批次 3：fees 寫入僅 PM/執行長 + fees_visible 移除 rateConfirmed 白名單
-- Scheme A：SQL token 對齊正式庫 schema_migrations.statements。

drop policy if exists fees_insert on public.fees;
create policy fees_insert on public.fees for insert with check ( is_admin((select auth.uid())) and env = current_env() );
drop policy if exists fees_update on public.fees;
create policy fees_update on public.fees for update using ( is_admin((select auth.uid())) and env = current_env() );
drop policy if exists fees_delete on public.fees;
create policy fees_delete on public.fees for delete using ( is_admin((select auth.uid())) and env = current_env() );
...
select
  ...
  case
    when is_admin((select auth.uid())) then f.client_info
      ...
      'clientTaskItems', '[]'::jsonb, 'sameCase', false, 'isFirstFee', false, 'notFirstFee', false,
      'client', '', 'contact', '', 'clientCaseId', '', 'eciKeywords', '', 'clientPoNumber', '',
      'clientCaseLink', jsonb_build_object('url', '', 'label', ''), 'dispatchRoute', '',
...
comment on view public.fees_visible is 'W10：fees 欄位遮罩 view（security_invoker）。讀取走此 view，寫入走 fees 原表。非管理員遮蔽 internal_note、client_info 營收客戶區塊（含 rateConfirmed，批次3 起一律清空）、edit_logs 只留白名單欄位條目（排除費率）。新增 fees 欄位需同步本 view。';
