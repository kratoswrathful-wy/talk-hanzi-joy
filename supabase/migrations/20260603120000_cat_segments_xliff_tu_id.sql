-- mqxliff / XLIFF 匯出：持久化 trans-unit id（與 id_value 分離；id_value 常為 x-mmq-context 路徑）

alter table public.cat_segments
  add column if not exists xliff_tu_id text;

comment on column public.cat_segments.xliff_tu_id is
  'XLIFF trans-unit id for export lookup; distinct from id_value (memoQ context path).';

-- Extend batch patch RPC (team refreshFileSegments)
...
comment on function public.apply_cat_segments_patch_batch is
  'Batch patch cat_segments rows in one RPC call. Includes xliff_tu_id for mqxliff export lookup.';
