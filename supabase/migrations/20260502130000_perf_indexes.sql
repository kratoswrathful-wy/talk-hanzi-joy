...
-- All indexes are created CONCURRENTLY (no table lock, no downtime).

-- cases: list query filters by env then sorts by created_at DESC
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_cases_env_created
  ON public.cases (env, created_at DESC);

-- cat_files: project view filters by project_id then sorts by created_at ASC
-- Replaces the less-specific single-column project_id index with a covering composite.
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_cat_files_project_created
  ON public.cat_files (project_id, created_at ASC);

-- cat_files: workspace list sorts by last_modified DESC (no filter)
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_cat_files_last_modified
