-- 工項 1：sync review 閘門改為 review_rows 非空 array（不再要求 multi_collab）
-- REPLACE sync_cat_workflow_assignments_for_case；無新表／新欄
-- 1) review_rows 為非空 array → 分段 upsert（寫 collab_row_id = row.id）＋清理
-- 2) review_rows 空／非 array → 不建 review 指派（cases.reviewer 僅顯示鏡像）
-- 3) translate 分支零變動；upsert 仍故意不傳 p_allow_downgrade
...
BEGIN
  ...
  -- ── review：review_rows 非空 array 即走分段（不要求 multi_collab）──
  IF jsonb_typeof(v_review_rows) = 'array' AND jsonb_array_length(v_review_rows) > 0 THEN
    ...
    -- 凡走 review_rows 路徑即清理（含單人）：stale id 或 collab_row_id IS NULL 皆刪
    FOREACH v_file_id IN ARRAY v_file_ids LOOP
    ...
    END LOOP;
  END IF;
  -- review_rows 空／非 array：不建指派（cases.reviewer 僅顯示鏡像）
...
COMMENT ON FUNCTION public.sync_cat_workflow_assignments_for_case(uuid) IS
  '工項1：review_rows 非空即分段 upsert（含單人）；清理 stale／NULL；不再讀 cases.reviewer；upsert 不傳 allow_downgrade';
